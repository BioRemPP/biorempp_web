"""
UC-5.6 Callbacks - Compound-Compound Functional Similarity Network.

This module implements callback functions for visualizing compound-compound
functional similarity through network analysis based on shared gene interactions.

Functions
---------
register_uc_5_6_callbacks
    Register all UC-5.6 callbacks with Dash app.

Notes
-----
- Refer to official documentation for use case details
- Uses NetworkStrategy in similarity mode for compound-compound relationships
- BioRemPP database required for compoundname and genesymbol data
...
def register_uc_5_6_callbacks(app, plot_service) -> None:
    """
    Register all UC-5.6 callbacks with Dash app.
    ...
    """
    logger.info("[UC-5.6] Registering callbacks")

    # ========================================
    # Callback 1: Toggle Informative Panel
    # ========================================
    @app.callback(
        Output("uc-5-6-collapse", "is_open"),
        Input("uc-5-6-collapse-button", "n_clicks"),
        State("uc-5-6-collapse", "is_open"),
        prevent_initial_call=True,
    )
    def toggle_uc_5_6_info_panel(n_clicks: Optional[int], is_open: bool) -> bool:
        """Toggle UC-5.6 informative panel collapse state."""
        if n_clicks:
            logger.debug(f"[UC-5.6] Toggling info panel: {is_open} -> {not is_open}")
            return not is_open
        return is_open
    ...
    # ========================================
    @app.callback(
        Output("uc-5-6-chart", "children"),
        [
            Input("merged-result-store", "data"),
            Input("uc-5-6-accordion-group", "active_item"),
        ],
        prevent_initial_call=True,
    )
    def render_uc_5_6(
        ...
        """
        Render UC-5.6 compound-compound similarity network on accordion activation.
        ...
        - Maps column names flexibly (compoundname/genesymbol with aliases)
        - Cleans data removing nulls and placeholder values
        - Passes prepared data to NetworkStrategy via PlotService
        - Computes compound-compound similarity based on shared genes
        - Generates similarity network diagram
        """
        logger.info(f"[UC-5.6] Render triggered, active_item: {active_item}")

        # Check if accordion is active
        if active_item != "uc-5-6-accordion":
            logger.debug("[UC-5.6] Accordion not active. Skipping render.")
            raise PreventUpdate
        ...
        try:
            ...
            # ========================================
            if not merged_data:
                logger.warning("[UC-5.6] merged_data is None or empty")
            ...
            if not isinstance(merged_data, dict):
                logger.error("[UC-5.6] merged_data is not a dictionary")
                return _create_error_message(
            ...
            if "biorempp_df" not in merged_data:
                logger.error("[UC-5.6] merged_data does not contain 'biorempp_df' key")
                return _create_error_message(
            ...
            # ========================================
            logger.debug("[UC-5.6] Extracting DataFrame from merged_data")
            biorempp_data = merged_data["biorempp_df"]

            if not biorempp_data:
                logger.warning("[UC-5.6] biorempp_df is empty")
                return _create_error_message(
            ...
            if df.empty:
                logger.warning("[UC-5.6] DataFrame is empty after conversion")
                return _create_error_message(
                    "No data available after processing.", "bi bi-inbox"
                )

            logger.info(
                f"[UC-5.6] Processing DataFrame: {len(df)} rows, "
                f"{len(df.columns)} columns"
            )
            logger.debug(f"[UC-5.6] Available columns: {df.columns.tolist()}")
            ...
            for col_name in compound_candidates:
                if col_name in df.columns:
                    ...
                    logger.debug(f"[UC-5.6] Mapped compoundname to '{col_name}'")
                    break
            ...
            for col_name in gene_candidates:
                if col_name in df.columns:
                    ...
                    logger.debug(f"[UC-5.6] Mapped genesymbol to '{col_name}'")
                    break

            # ========================================
            # Step 4: Validate required columns found
            # ========================================
            required = ["compoundname", "genesymbol"]
            missing_cols = [col for col in required if col not in col_map]

            if missing_cols:
                logger.error(
                    f"[UC-5.6] Missing columns: {missing_cols}. "
            ...
            # ========================================
            df_for_plot = df[[col_map["compoundname"], col_map["genesymbol"]]].rename(
                columns={
            ...
            logger.info(
                f"[UC-5.6] Data cleaned: {initial_count} -> {cleaned_count} rows"
            )

            if df_for_plot.empty:
                return _create_error_message(
                    "No valid compound-gene data found after cleaning.", "bi bi-funnel"
                )
            ...
            logger.info(
                f"[UC-5.6] Data statistics: "
                f"{n_compounds} unique compounds, {n_genes} unique genes"
            )

            # ========================================
            # Step 6: Generate plot using PlotService
            # ========================================
            logger.debug("[UC-5.6] Calling PlotService to generate similarity network")
            ...
            logger.info("[UC-5.6] Similarity network generation successful")
            ...
            try:
                suggested = sanitize_filename(
                    "UC-5.6", "compound_compound_network", "png"
                )
            except Exception:
                suggested = "compound_compound_network.png"
                        ...
                        [
                            html.Small(
                                [
                                    ...
                                    f"Network built from {n_compounds} compounds sharing "
                                    f"{n_genes} genes",
                                ],
            ...
            logger.error(
                f"[UC-5.6] ValueError during processing: {str(ve)}", exc_info=True
            )
            return _create_error_message(
            ...
            logger.error(f"[UC-5.6] Unexpected error: {str(e)}", exc_info=True)
            return _create_error_message(
                f"An unexpected error occurred: {str(e)}", "bi bi-bug"
            )

    logger.info("[UC-5.6] All callbacks registered successfully")
