"""
UC-5.4 Callbacks - Gene-Compound Interaction Network.

This module implements callback functions for visualizing gene-compound
interaction networks through bipartite network diagram analysis.

Functions
---------
register_uc_5_4_callbacks
    Register all UC-5.4 callbacks with Dash app.

Notes
-----
- Refer to official documentation for use case details
- Uses NetworkStrategy in bipartite mode for relationship visualization
...
def register_uc_5_4_callbacks(app, plot_service) -> None:
    """
    Register all UC-5.4 callbacks with Dash app.
    ...
    """
    logger.info("[UC-5.4] Registering callbacks")

    # ========================================
    # Callback 1: Toggle Informative Panel
    # ========================================
    @app.callback(
        Output("uc-5-4-collapse", "is_open"),
        Input("uc-5-4-collapse-button", "n_clicks"),
        State("uc-5-4-collapse", "is_open"),
        prevent_initial_call=True,
    )
    def toggle_uc_5_4_info_panel(n_clicks: Optional[int], is_open: bool) -> bool:
        """Toggle UC-5.4 informative panel collapse state."""
        if n_clicks:
            logger.debug(f"[UC-5.4] Toggling info panel: {is_open} -> {not is_open}")
            return not is_open
        return is_open
    ...
    # ========================================
    @app.callback(
        Output("uc-5-4-chart", "children"),
        [
            Input("merged-result-store", "data"),
            Input("uc-5-4-accordion-group", "active_item"),
        ],
        prevent_initial_call=True,
    )
    def render_uc_5_4(
        ...
        """
        Render UC-5.4 bipartite network on accordion activation.
        ...
        - Removes duplicates for cleaner network visualization
        - Passes prepared data to NetworkStrategy via PlotService
        - Generates bipartite gene-compound network diagram
        """
        logger.info(f"[UC-5.4] Render triggered, active_item: {active_item}")

        # Check if accordion is active
        if active_item != "uc-5-4-accordion":
            logger.debug("[UC-5.4] Accordion not active. Skipping render.")
            raise PreventUpdate
        ...
        try:
            ...
            # ========================================
            if not merged_data:
                logger.warning("[UC-5.4] merged_data is None or empty")
            ...
            if not isinstance(merged_data, dict):
                logger.error("[UC-5.4] merged_data is not a dictionary")
                return _create_error_message(
            ...
            if "biorempp_df" not in merged_data:
                logger.error("[UC-5.4] merged_data does not contain 'biorempp_df' key")
                return _create_error_message(
            ...
            # ========================================
            logger.debug("[UC-5.4] Extracting DataFrame from merged_data")
            biorempp_data = merged_data["biorempp_df"]

            if not biorempp_data:
                logger.warning("[UC-5.4] biorempp_df is empty")
                return _create_error_message(
            ...
            if df.empty:
                logger.warning("[UC-5.4] DataFrame is empty after conversion")
                return _create_error_message(
                    "No data available after processing.", "bi bi-inbox"
                )

            logger.info(
                f"[UC-5.4] Processing DataFrame: {len(df)} rows, "
                f"{len(df.columns)} columns"
            )
            logger.debug(f"[UC-5.4] Available columns: {df.columns.tolist()}")
            ...
            for col_name in gene_candidates:
                if col_name in df.columns:
                    ...
                    logger.debug(f"[UC-5.4] Mapped genesymbol to '{col_name}'")
                    break
            ...
            for col_name in compound_candidates:
                if col_name in df.columns:
                    ...
                    logger.debug(f"[UC-5.4] Mapped compoundname to '{col_name}'")
                    break
            ...
            if missing_cols:
                logger.error(
                    f"[UC-5.4] Missing columns: {missing_cols}. "
            ...
            # Remove duplicates for cleaner network
            df_for_plot = df_for_plot.drop_duplicates()

            cleaned_count = len(df_for_plot)
            logger.info(
                f"[UC-5.4] Data cleaned: {initial_count} -> {cleaned_count} rows"
            )

            if df_for_plot.empty:
                return _create_error_message(
                    "No valid gene-compound interactions found after cleaning.",
                    "bi bi-funnel",
                )
            ...
            # ========================================
            logger.debug("[UC-5.4] Calling PlotService to generate network diagram")
            ...
            logger.info("[UC-5.4] Network diagram generation successful")

            # ========================================
            # Step 7: Prepare download filename and return chart component
            # ========================================
            try:
                suggested = sanitize_filename("UC-5.4", "gene_compound_network", "png")
            except Exception:
                suggested = "gene_compound_network.png"
                        ...
                        [
                            html.Small(
                                [
                                    ...
                                    f"Network: {n_genes} genes, {n_compounds} compounds, "
                                    f"{cleaned_count} interactions",
                                ],
            ...
            logger.error(
                f"[UC-5.4] ValueError during processing: {str(ve)}", exc_info=True
            )
            return _create_error_message(
            ...
            logger.error(f"[UC-5.4] Unexpected error: {str(e)}", exc_info=True)
            return _create_error_message(
                f"An unexpected error occurred: {str(e)}", "bi bi-bug"
            )

    logger.info("[UC-5.4] All callbacks registered successfully")
