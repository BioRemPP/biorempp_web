...
This module provides callback functions for UC-4.13, handling dropdown
population and heatmap rendering based on compound pathway selection for
genetic profile analysis.

Callbacks
---------
register_uc_4_13_callbacks
    Register all UC-4.13 callbacks with Dash app.

toggle_uc_4_13_info_panel
    Toggle informative panel collapse.

initialize_compound_pathway_dropdown_uc_4_13
    Populate dropdown with unique compound pathways from HADEG data.

render_uc_4_13
    Generate heatmap showing gene-sample matrix for selected compound pathway.
...
Database Support:
- HADEG: ✅ Contains 'sample', 'Gene', 'compound_pathway', 'Pathway', 'ko'
- BioRemPP/KEGG/ToxCSM: ❌ Do NOT contain HADEG pathway data

Data Processing (inline):
- Filter by selected compound pathway
- Pass filtered data to PlotService
- HeatmapStrategy handles:
  * Groupby (Gene, sample) → nunique(ko)
  * Pivot to matrix (Gene × sample)
  * Sort by totals
  * Create heatmap
