"""
UC-3.5 Callbacks - Sample Similarity Based on Chemical Profiles.

This module implements callback functions for visualizing sample-sample similarity
through correlation analysis based on chemical compound interaction profiles.

Functions
---------
register_uc_3_5_callbacks
    Register all UC-3.5 callbacks with Dash app.
...
def register_uc_3_5_callbacks(app, plot_service) -> None:
    """
    Register all UC-3.5 callbacks with Dash app.
    ...
    """
    logger.info("[UC-3.5] Registering callbacks")

    # ========================================
    # Callback 1: Toggle Informative Panel
    # ========================================
    @app.callback(
        Output("uc-3-5-collapse", "is_open"),
        Input("uc-3-5-collapse-button", "n_clicks"),
        State("uc-3-5-collapse", "is_open"),
        prevent_initial_call=True,
    )
    def toggle_uc_3_5_info_panel(n_clicks: Optional[int], is_open: bool) -> bool:
        """
        Toggle UC-3.5 informative panel collapse state.
        ...
        """
        if n_clicks:
            logger.debug(f"[UC-3.5] Toggling info panel: {is_open} -> {not is_open}")
            return not is_open
        return is_open
    ...
    # ========================================
    @app.callback(
        Output("uc-3-5-chart", "children"),
        Input("uc-3-5-accordion-group", "active_item"),
    ...
    def render_uc_3_5(
        ...
        """
        Render UC-3.5 correlogram when accordion is activated.
        ...
        - Generates correlation matrix heatmap for chemical similarity
        """
        logger.debug(f"[UC-3.5] Render callback triggered. Active item: {active_item}")

        # Check if UC-3.5 accordion is active
        if not active_item or active_item != "uc-3-5-accordion":
            logger.debug("[UC-3.5] Accordion not active. Preventing update.")
            raise PreventUpdate
        ...
        try:
            ...
            # ========================================
            if not merged_data:
                logger.warning("[UC-3.5] merged_data is None or empty")
            ...
            if not isinstance(merged_data, dict):
                logger.error("[UC-3.5] merged_data is not a dictionary")
                return _create_error_message(
            ...
            if "biorempp_df" not in merged_data:
                logger.error("[UC-3.5] merged_data does not contain 'biorempp_df' key")
                return _create_error_message(
            ...
            # ========================================
            logger.debug("[UC-3.5] Extracting DataFrame from merged_data")
            biorempp_data = merged_data["biorempp_df"]

            if not biorempp_data:
                logger.warning("[UC-3.5] biorempp_df is empty")
                return _create_error_message(
            ...
            if df.empty:
                logger.warning("[UC-3.5] DataFrame is empty after conversion")
                return _create_error_message(
                    "No data available after processing.", "bi bi-inbox"
                )

            logger.info(
                f"[UC-3.5] Processing DataFrame: {len(df)} rows, "
                f"{len(df.columns)} columns"
            )
            logger.debug(f"[UC-3.5] Available columns: {df.columns.tolist()}")
            ...
            for col_name in sample_candidates:
                if col_name in df.columns:
                    ...
                    logger.debug(f"[UC-3.5] Mapped Sample to '{col_name}'")
                    break
            ...
            for col_name in compound_candidates:
                if col_name in df.columns:
                    ...
                    logger.debug(f"[UC-3.5] Mapped Compound_Name to '{col_name}'")
                    break
            ...
            if "Sample" not in col_map:
                ...
                logger.error(
                    f"[UC-3.5] Sample column not found. "
            ...
            if "Compound_Name" not in col_map:
                ...
                logger.error(
                    f"[UC-3.5] Compound_Name column not found. "
                    f"Available: {df.columns.tolist()}"
                )
            ...
            logger.debug(
                f"[UC-3.5] Using columns - "
                f"Sample: '{sample_col}', Compound: '{compound_col}'"
            )
            ...
            logger.info(
                f"[UC-3.5] Data cleaned: {initial_count} -> {cleaned_count} "
                f"rows ({initial_count - cleaned_count} removed)"
            )

            if df_for_plot.empty:
                logger.warning("[UC-3.5] No valid data after cleaning")
                return _create_error_message(
            ...
            if n_samples < 2:
                return _create_error_message(
                    f"Need at least 2 samples for correlation. " f"Found: {n_samples}",
                    "bi bi-exclamation-triangle",
                )
            ...
            # ========================================
            logger.debug("[UC-3.5] Calling PlotService to generate correlogram")

            fig = plot_service.generate_plot(
                use_case_id="UC-3.5", data=df_for_plot, filters={}, force_refresh=False
            )

            logger.info("[UC-3.5] Correlogram generation successful")
            ...
            try:
                suggested = sanitize_filename(
                    "UC-3.5", "sample_similarity_compound_correlogram", "png"
                )
            except Exception:
                suggested = "uc_3_5_sample_similarity_compound_correlogram.png"
            ...
            logger.error(
                f"[UC-3.5] ValueError during processing: {str(ve)}", exc_info=True
            )
            return _create_error_message(
            ...
            logger.error(f"[UC-3.5] Unexpected error: {str(e)}", exc_info=True)
            return _create_error_message(
                f"An unexpected error occurred: {str(e)}", "bi bi-bug"
            )

    logger.info("[UC-3.5] All callbacks registered successfully")
