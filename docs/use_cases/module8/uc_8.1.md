# UC-8.1 â€” Minimal Sample Grouping for Complete Compound Coverage

**Module:** 8 â€“ Assembly of Functional Consortia  
...
- **Key columns:**
  ...
  - `compoundclass` â€“ chemical class to which a compound belongs  
  - `compoundname` â€“ name of the chemical compound interacted with by the sample  
...
- **Annotation Redundancy**
  ...
  - **Few groups** â†’ high redundancy: many samples share similar compound co-annotation profiles.
  - **Many groups** â†’ low redundancy: compound annotation coverage relies on multiple distinct annotation profiles.
...
- **Methodological limitation**
  Set cover optimization operates strictly on presence/absence of annotations, ignoring the expression levels, metabolic fluxes, and physiological compatibility needed for real microbial consortia.

- **Visualization limitation**
  The grouping compresses identical annotation profiles into a single categorical axis, obscuring other biological differences (e.g., taxonomy, growth rates) among samples within the same group.

- **Interpretive limitation**
  A minimal set of sample groups covers the maximum annotation space, but this mathematical reduction does not confirm that these samples can coexist, cooperate, or effectively biodegrade the target compounds in vivo.
...
- **Annotation Interpretation**
  The analysis is based on **observed sampleâ€“compound co-annotations**. It does not encode kinetic parameters, environmental abundance, or interaction strength; it answers a structural annotation question: **Which distinct co-annotation profiles are minimally required to cover all compound co-annotations in a given class?**
