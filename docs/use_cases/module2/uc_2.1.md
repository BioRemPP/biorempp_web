...
# UC-2.1 â€” Ranking of Sample Functional Richness Across Databases
...
- **Key columns (per selected database):**
  ...
  - `ko` or `Gene` â€“ KEGG Orthology identifier (or equivalent column used to store KO IDs)
...
- **Methodological limitation**
  Rankings are based on unique KO counts per sample and do not incorporate abundance, expression state, pathway completeness, or experimental confirmation of activity.

- **Visualization limitation**
  The bar chart shows relative ranking and absolute counts for one selected database at a time, but does not expose which KO categories drive the differences between samples or databases.

- **Interpretive limitation**
  Rank shifts across BioRemPP, HADEG, and KEGG indicate differences in annotation coverage and curation scope, not confirmed functional specialization or superior biological performance.
