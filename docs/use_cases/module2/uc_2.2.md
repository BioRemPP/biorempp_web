# UC-2.2 â€” Ranking of Samples by Chemical Diversity
...
- **Key columns:**
  ...
  - `compoundname` â€“ name of the chemical compound associated with that sample
...
<a class="glightbox" href="../uc_2.2_result.png">
    <img src="../uc_2.2_result.png" alt="Representative output for UC-2.2">
</a>
...
- **Compound Association Breadth**
  Samples with taller/longer bars are co-annotated with a greater number of unique compounds in the dataset. This reflects the breadth of annotation-level compound associations and may serve as a basis for comparing samples, but does not confirm actual interaction capacity or metabolic versatility.

- **Comparative Ranking**
  The chart provides a straightforward ranking of samples by compound association count, which can help orient exploratory analyses and hypothesis generation about which samples show broader or narrower compound annotation coverage.

- **Narrow Association Profiles**
  Samples with shorter bars are co-annotated with fewer unique compounds in the dataset. This may reflect a more focused annotation profile, smaller dataset representation, or a narrower compound scope â€” not necessarily lower biological activity or relevance.
...
- **Methodological limitation**
  The metric counts unique compound names per sample and treats all co-annotations equally, without weighting by abundance, evidence strength, toxicity priority, or biodegradation relevance.

- **Visualization limitation**
  The ranking chart summarizes breadth of compound coverage but does not show which compounds are shared across samples, how strongly they are represented, or how compound classes are distributed.

- **Interpretive limitation**
  Samples with more associated compounds reflect broader annotation coverage in the current dataset, not confirmed interaction capacity, metabolic versatility, or remediation effectiveness.
...
- **Uniqueness Definition**
  Compound association count is defined as the **count of unique compounds per sample**. Multiple occurrences of the same `compoundname` for a given sample (e.g., across different reactions or genes) are counted only once.
