# UC-2.4 â€” Ranking of Compounds Richness by Gene Count per Chemical Classes 
...
- **Key columns:**
  - `compoundclass` â€“ categorical label defining the chemical class (e.g., Aromatics, Aliphatics, Chlorinated)
...
- **Low Gene Diversity**
  Compounds with shorter bars â€” especially those with a unique gene count of 1 â€” are co-annotated with fewer genes. This may reflect a narrower annotation profile or more limited co-occurrence data, rather than confirmed enzymatic simplicity.

- **Comparing Chemical Classes**
  By switching between chemical classes in the dropdown, users can compare gene co-annotation patterns across classes. This can help explore whether certain classes show broader gene diversity in their annotations â€” generating hypotheses about functional complexity differences that would require experimental follow-up to validate.
...
- **Methodological limitation**
  Unique gene counts per compound summarize annotation breadth but do not capture gene expression, copy number, enzyme kinetics, or whether distinct genes act in the same pathway.

- **Visualization limitation**
  The filtered ranking chart reduces each compound to a single gene-diversity count and does not reveal which genes contribute to the total or how those genes are functionally related.

- **Interpretive limitation**
  Compounds with more associated genes may suggest broader annotated functional context, but do not by themselves demonstrate mechanistic complexity, pathway completeness, or confirmed biotransformation.
