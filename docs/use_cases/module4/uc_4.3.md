# UC-4.3 â€” Functional Fingerprint of Pathway by Samples
...
- **Output structure:**
  - **Axes (Î¸):** one axis per `sample`, arranged around the circle  
...
4. **Rendering as Radar (Polar) Plot**
   - Each sample is mapped to an angular coordinate (Î¸) around the circle.  
...
<a class="glightbox" href="../uc_4.3_result.png">
    <img src="../uc_4.3_result.png" alt="Representative output for UC-4.3">
</a>
...
- **Methodological limitation**
  The radar axes plot KO annotation counts across samples, which inherently biases the visualization toward deeply sequenced samples or those with taxa overrepresented in reference databases.

- **Visualization limitation**
  The polygonal shape implies a cohesive functional profile, but the radial axes (samples) are completely independent, and the connecting lines have no biological meaning regarding sample-to-sample interactions.

- **Interpretive limitation**
  A larger radial expansion for specific samples indicates high annotation density for the selected pathway, but this geometric representation does not confirm synergistic interactions or actual ecological activity in a consortium.
