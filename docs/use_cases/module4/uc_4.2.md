# UC-4.2 â€” Ranking of Samples by Pathway Richness
...
- **Output structure:**
  ...
  - **Bar value:** pathway-level KO richness per sample (count of unique KOs for that `sample`â€“`pathname` pair)
...
- **Methodological limitation**
  Sample ranking relies on the absolute count of annotated KOs per pathway, ignoring crucial factors like gene copy number, transcriptional regulation, and varying substrate affinities across different taxa.

- **Visualization limitation**
  The vertical bar format arrays samples strictly by total annotation density, masking critical differences in which specific core vs. peripheral enzymes are actually present in each sample.

- **Interpretive limitation**
  Taller bars demonstrate broader genetic coverage for the pathway in the reference database, not a confirmed superiority in metabolic efficiency or biotransformation dominance in the environment.
