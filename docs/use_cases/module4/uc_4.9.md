# UC-4.9 â€” Profiling of Sample Enzymatic Activity
...
- **Key columns:**
  ...
  - `enzyme_activity` â€“ functional label for enzymatic activity (e.g., *oxidoreductase*, *transferase*)  
  - `genesymbol` â€“ gene symbols mapped to that enzymatic activity in a given sample

- **User control:**
  - **Dropdown â€“ Sample:** all unique `sample` identifiers available in the dataset.
...
- **Methodological limitation**
  Enzyme activity is ranked solely by the sheer number of unique annotated gene symbols, completely ignoring differences in enzyme kinetics, turnover rates, and substrate specificities.

- **Visualization limitation**
  The bar chart array aggregates diverse isoenzymes into a single monolithic block, hiding the fact that many of these genes might be mutually exclusive or expressed under conflicting environmental conditions.

- **Interpretive limitation**
  A tall bar highlights a high genetic redundancy for a specific enzyme class within the annotation database, not confirmed high catalytic output or essentiality for biotransformation.
