# UC-4.10 â€” Diversity of Enzymatic Activities Across Samples
...
- **Key columns:**
  ...
  - `enzyme_activity` â€“ enzymatic function label (e.g., *oxidoreductase*, *hydrolase*)  
  - `genesymbol` â€“ gene symbols assigned to that enzymatic activity in each sample
...
- **Bubbles â€“ Genetic Diversity per Activity per Sample**
...
- **Methodological limitation**
  Scaling bubble sizes by gene counts artificially emphasizes well-characterized enzyme classes with deep reference databases, while penalizing novel or under-studied degradation pathways.

- **Visualization limitation**
  The matrix condenses all unique genes associated with an enzyme activity into a single sphere, making it impossible to distinguish between a functional multi-subunit complex and a loose collection of redundant paralogs.

- **Interpretive limitation**
  The largest bubbles represent hotspots of dense genetic annotation, but this static mapping does not confirm robust in vivo expression or the actual remediation capacity of the sample.
