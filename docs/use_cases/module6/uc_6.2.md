# UC-6.2 â€” Biological Interaction Flow
...
**Primary outputs:** Multi-stage flow network from samples â†’ compound classes â†’ enzyme activities
...
- **Key columns:**
  ...
  - `compoundclass` â€“ chemical class/category of the compound
  - `enzyme_activity` â€“ functional label for the enzymatic activity (e.g., monooxygenase, dehydrogenase)
...
- **Broadly Co-annotated Enzyme Activities**
  - **Enzyme activity nodes** that receive flows from many different compound classes, or from many samples, may indicate **broadly co-annotated enzymatic functions**â€”appearing across diverse annotation contexts.
...
- **Methodological limitation**
  Flow thickness reflects the frequency of annotation records linking samples, compound classes, and enzymes, not the completeness or activity of degradation pathways.

- **Visualization limitation**
  The diagram aggregates flows through broad compound classes, hiding which specific chemicals are actually linked to which specific enzyme activities within a sample.

- **Interpretive limitation**
  Broad flows from a sample to an enzyme class indicate high database coverage, not necessarily that the sample relies predominantly on those enzymes for biodegradation.
