# UC-6.5 â€” Chemicalâ€“Enzymatic Hierarchy
...
**Primary outputs:** Hierarchical partitioning of substrate scope across chemical classes â†’ enzyme activities â†’ genes
...
- **Key columns:**
  ...
  - `enzyme_activity` â€“ functional label for the enzymatic activity  
  - `genesymbol` â€“ gene symbol or identifier associated with that activity  
...
3. **Aggregation of Compound Co-annotation Breadth**
   ...
   - this count represents the **compound co-annotation breadth** (number of unique compounds co-annotated) associated with that gene under that classâ€“activity context.
...
- **Broadly Co-annotated Genes**
  ...
  - genes co-annotated with many distinct compounds within a given classâ€“activity context,
...
- **Methodological limitation**
  Category area depends on the number of unique compound annotations, which may inflate the visual importance of chemical classes with many minor derivatives in the database.

- **Visualization limitation**
  The nested hierarchy shows aggregate compound counts but hides whether the specific genes listed are complete pathways or isolated, unrelated enzymes.

- **Interpretive limitation**
  Large areas assigned to specific gene symbols within an enzyme class denote high database co-occurrence, not confirmed broad-spectrum in vivo catalytic activity.
