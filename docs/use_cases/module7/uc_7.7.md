# UC-7.7 â€” Sample Risk Mitigation Depth

**Module:** 7 â€“ Toxicological Risk Assessment and Profiling  
**Visualization type:** Interactive treemap (hierarchical depth of geneâ€“compound interactions per sample)  
...
This use case focuses on the **depth** or **intensity** of KO annotation co-occurrences rather than the variety of compounds. The treemap builds a **KO Annotation Depth Profile** by quantifying, for each sample and toxicological category, how many **geneâ€“compound annotation co-occurrences** are recorded for predicted high-risk compounds. Larger areas correspond to more co-occurrences, which may highlight where each sample has concentrated annotation coverage and which compounds are associated with particularly large numbers of KO annotations.
...
- **Primary data sources:**
  - `BioRemPP_Results.xlsx or BioRemPP_Results.csv` â€“ KO annotation data linking samples, compounds, and genes
  - `ToxCSM.xlsx or ToxCSM.csv` â€“ predicted toxicity scores and qualitative labels for compounds
- **Key columns:**
  - From `BioRemPP_Results.xlsx or BioRemPP_Results.csv`:
    ...
    - `genesymbol` â€“ gene symbol or identifier associated with the interaction
  - From `ToxCSM.xlsx or ToxCSM.csv`:
    - `compoundname` â€“ chemical compound name (to be matched with BioRemPP)
    - `endpoint` / `label_*` â€“ endpoint-specific toxicity labels (e.g., "High Toxicity", "High Safety")
    - `supercategory` (derived) â€“ mapped toxicological super-category (e.g., Genomic, Environmental, Organic)
- **Hierarchy represented in the treemap:**
  - Level 1: **Sample**
  - Level 2: **Toxicity Category** (super-category)  
  - Level 3: **Compound Name**  

The quantitative value is the **total count of geneâ€“compound interactions** for each `(sample, category, compound)` path.
...
5. **Aggregation for Depth**
   ...
   For each unique path, the **total number of geneâ€“compound annotation co-occurrences** is computed (e.g., using `.size()` on the rows belonging to that path). This count is used as a measure of **KO annotation depth** for that path.
...
- **Area (Values)**
  The **area** of each rectangle is proportional to the **total number of geneâ€“compound annotation co-occurrences** associated with that `(sample, category, compound)` path. Larger areas indicate greater KO annotation depth.
...
- **High KO Annotation Depth**
  Large rectangles (especially at the compound level) may indicate **high KO annotation depth** for a sampleâ€“compound pair within a particular toxicological category. This reflects a large number of KO annotation co-occurrences (experimental validation required to confirm functional response).
...
- **Complementarity with Breadth (UC-7.6)**
  ...
  - UC-7.7 emphasizes *the total annotation co-occurrence count* per compoundâ€“sample pair.
...
- **Methodological limitation**
  Area scaling relies on total gene annotation counts, which may artificially inflate the prominence of promiscuous enzyme families or highly redundant annotations.

- **Visualization limitation**
  The nested rectangles map total annotation depth but obscure whether these genes form contiguous pathways or belong to isolated, unrelated biological processes.

- **Interpretive limitation**
  Large rectangles indicate high database redundancy for specific compound-sample pairs, not necessarily stronger catalytic efficiency or superior bioremediation potential.
...
- **Input Format**
  ...
  - `BioRemPP_Results.xlsx or BioRemPP_Results.csv` â€“ semicolon-delimited, containing at least `sample`, `compoundname`, and `genesymbol`, and  
  - `ToxCSM.xlsx or ToxCSM.csv` â€“ semicolon-delimited, containing `compoundname`, toxicity scores, labels, and endpoint metadata.
...
- **Depth Metric**
  ...
  - repeated geneâ€“compound annotation associations increase this count,
