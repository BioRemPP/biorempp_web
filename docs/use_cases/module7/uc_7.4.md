# UC-7.4 â€” Distribution of Toxicity Scores by Endpoint
...
- **Key columns and structure:**
  ...
  - associated metadata linking each `value_*` column to:
    ...
    - a **super-category** (e.g., `Gen` â†’ Genomic, `Env` â†’ Environmental)
...
<a class="glightbox" href="../uc_7.4_result.png">
    <img src="../uc_7.4_result.png" alt="Representative output for UC-7.4">
</a>
...
- **Identifying High-Risk Endpoints**
  Endpoints whose box plots are positioned **higher on the y-axis** (i.e., with higher medians) may represent areas of greater overall predicted toxicological concern. These endpoints could warrant increased attention when interpreting compound-level risk in subsequent analyses.
...
- **Spotting Potent Compounds**
  ...
  - prioritized investigation in downstream modules, and/or
...
- **Visualization limitation**
  Box plots summarize statistical spread and identify outliers but hide the identities of the specific chemical compounds driving those extremes.

- **Interpretive limitation**
  High median scores or numerous outliers indicate structural similarity to known toxicants within a class, not confirmed adverse outcomes or actual risk in situ.
