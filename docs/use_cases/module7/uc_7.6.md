# UC-7.6 â€” Sample Risk Mitigation Breadth
...
- **Breadth vs. Depth**
  This use case emphasizes **breadth of co-annotation coverage**â€”the variety of high-risk compounds co-annotatedâ€”rather than **depth** (e.g., number of KO annotations per compound). It is therefore complementary to analyses that focus on annotation counts and annotation diversity.
...
- **Methodological limitation**
  Area scaling is driven by raw counts of unique compounds, inherently favoring well-studied toxicological categories regardless of actual sample activity or pathway completeness.

- **Visualization limitation**
  The nested rectangles display aggregate compound variety but hide the specific functional diversity, completeness, or biological relevance of the underlying genes.

- **Interpretive limitation**
  Large rectangles highlight extensive annotation overlap with high-risk chemical categories in the database, not confirmed broad-spectrum detoxification capacity or survival advantage.
...
- **Input Format**
  ...
  - `BioRemPP_Results.xlsx or BioRemPP_Results.csv` â€“ semicolon-delimited, containing at least `sample` and `compoundname`, and  
...
- **Breadth Metric**
  The visualization uses the **count of unique high-risk compound names per (sample, category)** as its driving value. Multiple occurrences of the same compound for a given sampleâ€“category combination are counted only once.
