# UC-3.4 â€” Sample Similarity (Based on KO Profiles) 

**Module:** 3 â€“ System Structure: Clustering, Similarity, and Co-occurrence  
**Visualization type:** Correlogram (sample Ã— sample similarity heatmap in KO space)  
...
<a class="glightbox" href="../uc_3.4_result.png">
    <img src="../uc_3.4_result.png" alt="Representative output for UC-3.4">
</a>
...
- **KO Annotation Clusters**
  Blocks or patches of warm colors off the main diagonal may indicate **clusters of samples with highly similar KO annotation profiles**. Such clusters could correspond to samples sharing comparable KO annotation repertoires and may warrant joint investigation.

- **Distinct Annotation Groups**
  The large-scale pattern of the heatmap can reveal **annotation-based distinct groups**:
...
- **Unique Annotation Profiles**
  Samples that show generally low similarity (cooler colors) across their row/column may have **unique or distinct KO annotation profiles** in the context of this dataset, making them potential candidates for focused investigation.
...
- **Methodological limitation**
  Pairwise similarities are calculated with Pearson correlation on binary KO presence/absence vectors, which captures linear co-variation but is sensitive to sparse profiles and does not model pathway weighting or abundance.

- **Visualization limitation**
  The correlogram compresses each sample pair into a single similarity value, so it does not reveal which KO subsets create a high or low correlation pattern.

- **Interpretive limitation**
  High or low correlation indicates annotation-level similarity or divergence, not direct evidence of shared metabolism, ecological interaction, or consortium compatibility.
...
- **Interpretation Scope**
  The correlogram reflects similarity in **KO annotation profiles**, not expression levels, gene copy numbers, or kinetic parameters. These aspects require additional layers of experimental data and analysis beyond the presence/absence of KO identifiers.
