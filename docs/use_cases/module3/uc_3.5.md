# UC-3.5 â€” Sample Similarity (Based on Chemical Profiles)

**Module:** 3 â€“ System Structure: Clustering, Similarity, and Co-occurrence  
**Visualization type:** Correlogram (sample Ã— sample similarity heatmap in compound space)  
...
- **Methodological limitation**
  Similarities are computed from binary compound presence/absence profiles using Pearson correlation and therefore do not capture compound abundance, annotation confidence, or chemical importance.

- **Visualization limitation**
  The heatmap reduces each pair of samples to a single compound-profile similarity score and does not expose which compound classes or specific compounds drive the pattern.

- **Interpretive limitation**
  Correlated compound profiles indicate overlapping annotation coverage, not confirmed shared exposure history, functional interaction capacity, or equivalent remediation behavior.
