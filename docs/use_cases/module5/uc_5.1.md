# UC-5.1 â€” Sample-Compound Class Interaction

**Module:** 5 â€“ Modeling Interactions of Samples, Genes, and Compounds  
**Visualization type:** Chord diagram (bipartite sampleâ€“compound-class interaction network)  
**Primary inputs:** BioRemPP results table with `sample` and `compoundclass` columns  
**Primary outputs:** Interaction matrix of samples Ã— compound classes (co-occurrence counts)
...
- **Key columns:**
  ...
  - `compoundclass` â€“ categorical label for the chemical class of each compound
...
- **Derived structure:** interaction matrix with:
  ...
  - cell = interaction count for each sampleâ€“class pair
...
- **Chords (Ribbons)**
  The ribbons spanning between arcs represent individual **Sampleâ€“Compound Class** interactions:
  - one end of the ribbon is anchored at a sample arc,  
  - the other end at a compound class arc.

- **Chord Thickness**
  The thickness of a chord at its connection points is proportional to the **interaction count** for that sampleâ€“class pair:
...
<a class="glightbox" href="../uc_5.1_result.png">
    <img src="../uc_5.1_result.png" alt="Representative output for UC-5.1">
</a>
...
- **Strong Co-annotation Associations**
  A thick chord between a specific sample and a given compound class may indicate a **strong co-annotation frequency**:
...
- **Highly Co-annotated Chemical Classes**
...
- **Co-annotation Landscape Overview**
  The overall pattern of chords can provide a **global view** of how co-annotations are distributed:
...
- **Methodological limitation**
  Co-occurrence counts reflect the number of database records linking a sample's annotations to a chemical class, without accounting for gene expression, abundance, or completeness of degradation pathways.

- **Visualization limitation**
  The chord diagram shows total aggregate connections but hides which specific compounds or gene annotations drive the interactions between each sample and class.

- **Interpretive limitation**
  Strong connections indicate broad annotation overlap with a chemical class in the database, not confirmed biotransformation capacity or ecological function.
...
- **Scope of Interpretation**
  The chord diagram summarizes **co-annotation frequency**, not mechanistic detail:
