"""Smoke tests for Module 5 use case layout composition."""
...
@pytest.mark.parametrize(
    ("factory", "use_case_id"),
    [
        (create_uc_5_1_layout, "uc-5-1"),
        (create_uc_5_2_layout, "uc-5-2"),
        (create_uc_5_3_layout, "uc-5-3"),
        (create_uc_5_4_layout, "uc-5-4"),
        (create_uc_5_5_layout, "uc-5-5"),
        (create_uc_5_6_layout, "uc-5-6"),
    ],
)
def test_module5_layout_places_info_panel_before_results(
    factory: Callable[[], dbc.Card], use_case_id: str
) -> None:
    """Each Module 5 card should keep the info panel before the results accordion."""
    layout = factory()
